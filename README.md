...
Today the gods are old and rusty and need the human's help for Ragnarok. This is where our game begins. The gods want to find the strongest humans, so they challenge them and see if someone manages to climb the world tree Yggdrasil, overcoming all obstacles and defeating the final boss.

# Goal of the game
The game starts with a tutorial that will introduce the various components of the game. Initially, after the tutorial concludes, the players will have to move their ghosts to register in the current game. Afterwards, the players can simply move into one of the displayed characters to select it as their game character.

When all players (max 4, min 1) have selected their character, they can step onto the START button. Once all registered players step on it, the game will start by lowering the first set of doors to the next rooms of the level.

The players can then freely move from room to room to clear a path to the top of the tree. This tree along with all uncovered rooms can be seen in the pause menu as well. At the top of the tree, somewhere, in different possible places, the final challenge is located that has to be beaten by the players to win the game.

In the final room there is a button named Escape, that the players can step on in case they don't feel ready for the challenge yet. This feature can be used only once during a play-through. It is useful if players want to collect more items in the rest of the rooms to come back stronger.
...
# Encounters
The moving-to-the-top is divided into various encounters. An encounter starts as soon as all players that are not currently ghosts are inside the same room (Note that there is a small offset of 1 tile between the exit of a corridor and the room). Before an Encounter starts, the players can see the enemies in a frozen state but can't shoot them. The encounter starts by closing off all exits to that room, so that the players are trapped until they kill all the enemies inside that room.

After a room is cleared, it stays that way until the end.

# Freeroam
Between encounters, players are free to move around in all cleared rooms. The camera will continously zoom in and out to keep all alive players in the view.
...
# Ghosts
If a player dies during the game, they will turn into a ghost that can't do anything. However, it can collect a revive item (looking like a big Plus) that will revive the player again. Despite that, death comes at a price: The player loses their current weapon and will instead get a starter weapon again.
...
# Controls
The controls are explained during the tutorial. The image shown below is also always displayed in the start room. If the players press the pause button during the game, the camera will zoom out and show the entire tree including the so-far uncovered rooms.
![teaser image](controller.jpg)

### Keyboard steering
The fourth player can always additionally be controlled using the keyboard and mouse. This allows one extra player to play in case a party does not have four controllers available. The controls are **WASD**. Shooting is with the **left mouse button** in direction of the mouse pointer. Special abilities are triggered with **LeftAlt**. Press **SPACE** to dash. **Note** that this is supposed to facilitate playing when controllers are not available, but that it is much easier because one can simply point the mouse at an enemy and shoot while with a controller aiming is harder.
...
These additional shortcuts are intended for debugging, and are only available if the debug mode is enabled in settings.
...
* **Left-click**: teleport players to location (only in manual Camera Mode)
* **Backspace**: enable god-mode for all players
* **Delete**: kill all non-boss enemies in the game
